 ...
 * @author Joakim von Kistowski, Martin Straesser
 */
public class HTTPInputGenerator {
	 ...
	 * @param method Will create a POST request if this parameter equals "POST" (ignoring case)
	 *               and a GET request otherwise
	 * @param payload Will add the associated String as JSON payload if present and a POST request
	 *                is initialized
	 * @return The http client's initialized request.
	 */
	public Request initializeHTTPRequest(String url, String method, Optional<String> payload) {
		Request request;
		if (method.equalsIgnoreCase("POST")) {
			 request = httpClient.POST(url);
			if (payload.isPresent()) {
				request.content(new StringContentProvider(payload.get()), "application/json");
			}
		} else {
