...
set +e
# Admin Login
curl -X POST -H "Content-Type: application/json" -d '{"name": "adminroot","password": "adminroot"}' http://ts-ui-dashboard:8080/account/adminlogin > temp.txt
...
for ((i = 1 ; i <= $1 ; i++)); do
    ...
    curl -X POST -H "Content-Type: application/json" -d "$payload" http://ts-ui-dashboard:8080/adminuser/addUser
done
set -e
